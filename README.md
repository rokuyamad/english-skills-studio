...
`+ New` や選択導線で `term_en / term_ja / example_en / example_ja` が揃っていれば、その場で `ready` 保存され、`srs_card_states` が `en_to_ja` / `ja_to_en` の2方向で自動作成されます。

既存の `ready` カードに state 欠落がある環境では、最新 migration を適用して backfill してください。
