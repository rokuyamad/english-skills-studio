...
| `scripts/import-shadowing-set.py` | 公開 Notion ページ → Shadowing セット抽出 + JSON upsert |
...
## 新しい Shadowing セットを追加する手順

1. 公開 Notion ページ URL を用意
   - 対象は各セクションの `YouTubeはこちら` に埋め込まれた YouTube 動画
2. 反映前に確認したい場合は dry-run を実行
...
4. `data/shadowing-data.json` が source URL ベースで upsert される
5. `http://localhost:8080/shadowing.html` でセット表示を確認
...
- `SHADOWING:DRYRUN <url>`
  - 実行: `uv run python3 scripts/import-shadowing-set.py --url "<url>" --dry-run`
  - 目的: 公開 Notion の Shadowing セット抽出結果を確認（JSON更新なし）
- `SHADOWING:IMPORT <url>`
  - 実行: `uv run python3 scripts/import-shadowing-set.py --url "<url>"`
  - 目的: 公開 Notion の Shadowing セットを抽出して `data/shadowing-data.json` に反映

補足:
- `SRS:RUN` / `SRS:APPLY` は `SUPABASE_URL` / `SUPABASE_SERVICE_ROLE_KEY` が環境変数に設定済みであることを前提とする。
- `SHADOWING:DRYRUN` / `SHADOWING:IMPORT` は追加の環境変数不要。
